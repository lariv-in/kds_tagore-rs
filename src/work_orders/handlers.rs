...
use super::{
    entities::{
        component, draft_work_order_material_line, machine, material, material_rate,
        proforma_invoice, proforma_invoice_machine_line, proforma_invoice_material_line, shape,
...
pub async fn work_orders_list(
    ...
    let orders_with_stats: Vec<(work_order::Model, usize, Decimal)> = orders
        ...
        .map(|o| {
            ...
            let total = o.total_amount(lines);
            let count = lines.len();
            (o, count, total)
        })
        .collect();
...
pub async fn work_order_detail(
    ...
    let total_amount = order.total_amount(&lines);
...
pub async fn work_order_create_post(
    ...
    if form.order_number.trim().is_empty() {
        let components = fetch_components_meta(&state.db).await;
        let components_json = serde_json::to_string(&components).unwrap_or_else(|_| "[]".into());
        let page = WorkOrderCreateModalPage {
            form_name: q.form_name(),
            order_number: form.order_number,
            customer_id: if form.customer_id > 0 { Some(form.customer_id) } else { None },
            customer_name: String::new(),
            items_json: form.items.unwrap_or_default(),
            components_json,
            error: "Order number is required.".into(),
        };
...
#[derive(Debug, Clone, Deserialize)]
#[serde(rename_all = "PascalCase")]
pub struct WorkOrderLineCreateForm {
...
pub async fn work_order_line_create_get(
    Cap(state): Cap<WorkOrdersState>,
    Cap(chrome): Cap<SharedChromeFolder>,
    auth: OptionalAuth,
    Query(q): Query<ModalFormQuery>,
    ...
    let order = work_order::Entity::find_by_id(id).one(&state.db).await.ok().flatten();
    let label = order.map(|o| format!("{} (#{})", o.order_number, o.id)).unwrap_or_else(|| format!("#{}", id));

    let page = WorkOrderLineCreateModalPage {
        draft_work_order_id: id,
...
pub async fn work_order_line_create_post(
    Cap(state): Cap<WorkOrdersState>,
    Cap(chrome): Cap<SharedChromeFolder>,
    auth: OptionalAuth,
    htmx: Htmx,
    Query(q): Query<ModalFormQuery>,
    Path(id): Path<i64>,
    Form(form): Form<WorkOrderLineCreateForm>,
) -> Response {
    let slot_ctx = auth.0.as_ref().map(SlotCtx::from_auth).unwrap_or_default();
    let target_order_id = form.draft_work_order_id.unwrap_or(id);
    ...
    if form.quantity.trim().is_empty() {
        ...
        let page = WorkOrderLineCreateModalPage {
            ...
            draft_work_order_label: label,
            form_name: q.form_name(),
            component_id: Some(form.component_id),
    ...
    match resolve_and_compute_line_data(
        ...
        Ok((vars_json, quantity, unit_weight, material_rate, final_cost, extra_data)) => {
            let now = Utc::now();
            let model = draft_work_order_material_line::ActiveModel {
                id: Default::default(),
                created_at: Set(Some(now)),
                updated_at: Set(Some(now)),
                draft_work_order_id: Set(target_order_id),
                component_id: Set(form.component_id),
                variables: Set(vars_json),
            ...
            match model.insert(&state.db).await {
                Ok(_) => respond_create_modal_done::<WorkOrderLineCreateModalKey>(
                    &htmx,
                    &q.refresh_table(),
